...
cdf_file = 'assets\COPERNICUS_DWNLDS\SSH_COP_FEB_26_2024_PERU.nc'
cdf_data = cdf.Dataset(cdf_file)

counter = 0


for i in range(len(np.array(cdf_data['zos']))):
    temp = np.array(cdf_data['zos'])[i]
    temp[temp == 9.96921e+36] = np.NaN
    ...
    plot_tsm = ax.contourf(lon, lat, temp, levels=np.linspace(-.6,.6, 25) , cmap = 'coolwarm')
    ...
    plt.savefig('IMGS/SSH/SSH_COP_'+ str(counter) + '_PERU_2024_ENE')
    plt.close()
    counter += 1
